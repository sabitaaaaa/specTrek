    ...
    <table class="table table-bordered">
        ...
        @foreach($trekPackages as $trek)
            <tr>
                ...
                <td>
                    <a href="{{ route('trek-packages.edit', $trek->id) }}" class="btn btn-sm btn-warning">✏ Edit</a>
                    <form action="{{ route('trek-packages.destroy', $trek->id) }}" method="POST" style="display:inline-block;">
                        @csrf @method('DELETE')
                        <button class="btn btn-sm btn-danger" onclick="return confirm('Delete this trek?')">🗑 Delete</button>
                    </form>
                </td>
            </tr>
        @endforeach
